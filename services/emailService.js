...
/** Şifrə dəyişikliyi üçün 2FA kodu şablonu (İngilis dilində) */
function passwordChangeHtml(code, minutes = 3) {
  const digits = String(code)
    .split('')
    .map(
      (d) => `<td style="padding:0 5px;">
...
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body style="margin:0;padding:0;background:#ffffff;font-family:Arial,Helvetica,sans-serif;">
  <div style="display:none;max-height:0;overflow:hidden;">Your ${SITE_NAME} password change code is ${code}</div>
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#ffffff;padding:28px 12px;">
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;">
        <tr><td align="center" style="padding-bottom:20px;">
        ...
        <tr><td style="background:#3d0d62;border-radius:20px;padding:30px 26px;color:#ffffff;">
          <div style="font-size:20px;font-weight:800;margin-bottom:10px;">Confirm your password change</div>
          <div style="font-size:14px;line-height:1.6;color:rgba(255,255,255,.85);">
            A password change was requested for your ${SITE_NAME} account. Enter the code below to confirm it.
          </div>
          <table role="presentation" cellpadding="0" cellspacing="0" align="center" style="margin:24px auto 14px;">
            <tr>${digits}</tr>
          </table>
          ...
          <div style="margin-top:22px;padding:12px 14px;border-radius:12px;background:rgba(255,255,255,.08);
                      font-size:12.5px;line-height:1.6;color:rgba(255,255,255,.8);">
            If you did NOT request a password change, do not enter this code — your password stays unchanged.
            Never share this code with anyone; ${SITE_NAME} staff will never ask for it.
          </div>
        </td></tr>
        <tr><td align="center" style="padding-top:18px;font-size:11px;line-height:1.7;color:#9a9a9a;">
          &copy; ${new Date().getFullYear()} ${SITE_NAME} &middot; 18+ &middot; Play responsibly
        </td></tr>
      </table>
    </td></tr>
  </table>
...
/** Şifrə dəyişikliyi kodunu göndərir (2FA) */
async function sendPasswordChangeCode(email, code) {
  const minutes = Math.round(CODE_TTL_MS / 60000);
  return sendMail({
    to: email,
    subject: `${code} is your ${SITE_NAME} password change code`,
    html: passwordChangeHtml(code, minutes),
    text: [
      `${SITE_NAME} — password change verification`,
      '',
